...
typedef struct {
	PhoneInfo* tel[SIZE];// 포인터 배열이 각각의 구조체를 가지고 있음.
...
int SearchByName(PhoneBook *k) {
	...
	print(2); // 검색완료 print
	for (i = 0; i < k->n; i++) {
		if (strcmp(name2, k->tel[i]->Name) == 0) {
			printf("\n이름 : %s\n", k->tel[i]->Name);
			printf("나이 : %d\n", k->tel[i]->age);
			printf("전화번호 : %s\n", k->tel[i]->TelNumber);
			printf("\n");
	...
	if (i == k->n) { // i가 k->n -1 일꺼 같지만 k->n 이여야함..
		printf(" 검색결과가 없습니다 !!\n");
		printf(" %d초 후 메뉴로 돌아갑니다 !!\n", DELAY / 1000);
		Sleep(DELAY);
	}
	return i;
}
...
void insert(PhoneBook *k) { // 전화번호 추가하기.
	...
	printf("    <    추     가     하     기    >\n");
	printf(" ─────────────────── \n");
	if (k->n == SIZE) { // 삽입하려는데 전화번호부가 꽉찼을 때.
		printf("전화번호부가 꽉찼습니다\n");
		printf(" ─────────────────── \n");
	}
	else {
		k->tel[k->n] = (PhoneInfo *)malloc(sizeof(PhoneInfo)); // 구조체 동적할당생성.
		printf("\n현재 저장된 사람 %d / %d\n", k->n, SIZE);
		...
		k->tel[(k->n)]->age = age2;
		k->tel[k->n]->Name = (char *)malloc(sizeof(strlen(name2))); // 이름 동적할당.
